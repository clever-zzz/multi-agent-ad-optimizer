...
def _severity_from_gap(
    observed: float, threshold: float, *, higher_is_worse: bool
) -> AlertSeverity:
...
def _check_ctr(snapshot: PerformanceSnapshot, cfg: AlertThresholds) -> list[Alert]:
    ...
    return [
        Alert(
            ...
            severity=_severity_from_gap(snapshot.ctr, cfg.ctr_floor, higher_is_worse=False),
...
def _check_cpa(snapshot: PerformanceSnapshot, cfg: AlertThresholds) -> list[Alert]:
    ...
    return [
        Alert(
            ...
            severity=_severity_from_gap(cpa, cfg.cpa_ceiling, higher_is_worse=True),
...
def _check_roas(snapshot: PerformanceSnapshot, cfg: AlertThresholds) -> list[Alert]:
    ...
    return [
        Alert(
            ...
            severity=_severity_from_gap(snapshot.roas, cfg.roas_floor, higher_is_worse=False),
...
def _check_burn_rate(
    ...
    return [
        Alert(
            ...
            if ratio >= cfg.burn_rate_multiplier * 1.5
...
def _check_frequency(
    ...
    return [
        Alert(
            ...
            severity=_severity_from_gap(frequency, cfg.frequency_ceiling, higher_is_worse=True),
